...
namespace JiiLib.Media.Internal
{
    ...
    /// </summary>
    /// <remarks>
    ///     Taken from http://stackoverflow.com/a/24618827,
    ///     slightly modified to use C# 6 features.
    /// </remarks>
    internal sealed class Latin1EncodingFallback : Encoding
    {
        public static Latin1EncodingFallback Instance { get; } = new Latin1EncodingFallback();

        private readonly string _specialCharset = (char)0xA0 + @"¡¢£¤¥¦§¨©ª«¬­®¯°±²³´µ¶·¸¹º»¼½¾¿ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖ×ØÙÚÛÜÝÞßàáâãäåæçèéêëìíîïðñòóôõö÷øùúûüýþÿ";
        ...
        private Latin1EncodingFallback() {  }
        ...
        {
            if (chars is null) throw new ArgumentNullException(nameof(chars), @"null array");
            if (bytes is null) throw new ArgumentNullException(nameof(bytes), @"null array");
            ...
            for (int i = 0; i < charCount; i++)
            {
                ...
                int chVal = ch;
                bytes[byteIndex + i] = chVal < 160 ? (byte)ch : (chVal <= Byte.MaxValue ? (byte)_specialCharset[chVal - 160] : (byte)63);
            }
        ...
        public override int GetChars(byte[] bytes, int byteIndex, int byteCount, char[] chars, int charIndex)
        {
            ...
            for (int i = 0; i < byteCount; ++i)
            {
                ...
                chars[charIndex + i] = b < 160 ? (char)b : _specialCharset[b - 160];
            }
